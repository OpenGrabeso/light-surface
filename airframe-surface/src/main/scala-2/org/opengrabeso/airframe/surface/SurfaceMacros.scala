...
private[surface] object SurfaceMacros {
  def surfaceOf[A: c.WeakTypeTag](c: sm.Context): c.Tree = {
    ...
    if (t.isStatic) {
      q"org.opengrabeso.airframe.surface.SurfaceFactory.of[${targetType}]"
    } else {
      ...
      owner match {
        case o if o.isClass && o != NoSymbol && !o.isAbstract =>
          q"org.opengrabeso.airframe.surface.SurfaceFactory.localSurfaceOf[${targetType}](${t.owner}.this)"
        case _ =>
          q"org.opengrabeso.airframe.surface.SurfaceFactory.localSurfaceOf[${targetType}](this)"
      }
    }
  }
  ...
  def methodSurfaceOf[A: c.WeakTypeTag](c: sm.Context): c.Tree = {
    import c.universe.*
    val targetType = implicitly[c.WeakTypeTag[A]].tpe
    q"org.opengrabeso.airframe.surface.SurfaceFactory.methodsOf[${targetType}]"
  }
  ...
  private[surface] class SurfaceGenerator[C <: sm.Context](val c: C) {
    ...
    def createMethodSurfaceOf(targetType: c.Type): c.Tree = {
      if (methodMemo.contains(targetType)) {
        ...
        val expr =
          q"org.opengrabeso.airframe.surface.methodSurfaceCache.getOrElseUpdate(${fullName}, ${result})"
    ...
    private val taggedTypeFactory: SurfaceFactory = {
      case t if t.typeArgs.length == 2 && isTaggedType(t) =>
        ...
        q"org.opengrabeso.airframe.surface.TaggedSurface(${surfaceOf(typeArgs(0))}, ${surfaceOf(typeArgs(1))})"
    }
    ...
    private val aliasFactory: SurfaceFactory = {
      case alias @ TypeRef(prefix, symbol, args)
        ...
        q"org.opengrabeso.airframe.surface.Alias(${name}, ${fullName}, $inner)"
    }
    ...
    private val higherKindedTypeFactory: SurfaceFactory = {
      case t @ TypeRef(prefix, symbol, args) if t.typeArgs.isEmpty && t.takesTypeArgs =>
        ...
        q"org.opengrabeso.airframe.surface.HigherKindedTypeSurface(${name}, ${fullName}, ${inner}, Seq.empty)"
      case t @ TypeRef(NoPrefix, tpe, List()) if tpe.name.decodedName.toString.contains("$") =>
        q"org.opengrabeso.airframe.surface.ExistentialType"
      case t @ TypeRef(NoPrefix, tpe, args) if !t.typeSymbol.isClass =>
        ...
        q"org.opengrabeso.airframe.surface.HigherKindedTypeSurface(${name}, ${fullName}, ${inner}, Seq(..${typeArgs}))"
    }

    private val primitiveFactory: SurfaceFactory = {
      case t if t == typeOf[Short] => q"org.opengrabeso.airframe.surface.Primitive.Short"
      case t if t == typeOf[Boolean] =>
        q"org.opengrabeso.airframe.surface.Primitive.Boolean"
      case t if t == typeOf[Byte]  => q"org.opengrabeso.airframe.surface.Primitive.Byte"
      case t if t == typeOf[Char]  => q"org.opengrabeso.airframe.surface.Primitive.Char"
      case t if t == typeOf[Int]   => q"org.opengrabeso.airframe.surface.Primitive.Int"
      case t if t == typeOf[Float] => q"org.opengrabeso.airframe.surface.Primitive.Float"
      case t if t == typeOf[Long]  => q"org.opengrabeso.airframe.surface.Primitive.Long"
      case t if t == typeOf[Double] =>
        q"org.opengrabeso.airframe.surface.Primitive.Double"
      case t if t == typeOf[String] =>
        q"org.opengrabeso.airframe.surface.Primitive.String"
      case t if t == typeOf[java.lang.String] =>
        q"org.opengrabeso.airframe.surface.Primitive.String"
      case t if t == typeOf[Unit]                 => q"org.opengrabeso.airframe.surface.Primitive.Unit"
      case t if t == typeOf[BigInt]               => q"org.opengrabeso.airframe.surface.Primitive.BigInt"
      case t if t == typeOf[java.math.BigInteger] => q"org.opengrabeso.airframe.surface.Primitive.BigInteger"
    }

    private val arrayFactory: SurfaceFactory = {
      case t if typeNameOf(t) == "scala.Array" =>
        q"org.opengrabeso.airframe.surface.ArraySurface(classOf[$t], ${elementTypeOf(t)})"
    }

    private val optionFactory: SurfaceFactory = {
      case t if typeNameOf(t) == "scala.Option" =>
        q"org.opengrabeso.airframe.surface.OptionSurface(classOf[$t], ${elementTypeOf(t)})"
    }
    ...
    private val tupleFactory: SurfaceFactory = {
      case t if t <:< typeOf[Product] && t.typeSymbol.fullName.startsWith("scala.Tuple") =>
        ...
        q"new org.opengrabeso.airframe.surface.TupleSurface(classOf[$t], IndexedSeq(..$paramType))"
    }
    ...
    private val javaUtilFactory: SurfaceFactory = {
      case t
        ...
        q"new org.opengrabeso.airframe.surface.GenericSurface(classOf[$t])"
    }

    private val javaEnumFactory: SurfaceFactory = {
      case t if isEnum(t) =>
        q"org.opengrabeso.airframe.surface.JavaEnumSurface(classOf[$t])"
    }
    ...
    def methodParametersOf(targetType: c.Type, method: MethodSymbol, args: Seq[MethodArg]): c.Tree = {
      ...
      val ref =
        q"org.opengrabeso.airframe.surface.MethodRef(${toClassOf(targetType)}, ${method.name.decodedName.toString}, Seq(..$argTypes), ${method.isConstructor})"
    ...
    private def newGenericSurfaceOf(t: c.Type): c.Tree = {
      val expr =
        q"new org.opengrabeso.airframe.surface.GenericSurface(classOf[${finalTypeOf(t)}])"
      expr
    }
    ...
    private val genericSurfaceFactory: SurfaceFactory = {
      case t if t =:= typeOf[Any] =>
        q"org.opengrabeso.airframe.surface.Alias(${"Any"}, ${"scala.Any"}, org.opengrabeso.airframe.surface.AnyRefSurface)"
      case t @ TypeRef(prefix, symbol, args) if !args.isEmpty =>
        val typeArgs = typeArgsOf(t).map(surfaceOf(_))
        q"new org.opengrabeso.airframe.surface.GenericSurface(classOf[$t], typeArgs = IndexedSeq(..$typeArgs))"
      ...
      case t if hasStringUnapply(t) =>
        t.typeSymbol.fullName.split("\\.").toList match {
          case p1 :: tl =>
            ...
            val expr =
              q"""org.opengrabeso.airframe.surface.EnumSurface(classOf[${finalTypeOf(t)}], { (cl: Class[_], s: String) =>
    ...
    def surfaceOf(t: c.Type): c.Tree = {
      if (seen.contains(t)) {
        if (memo.contains(t)) {
          ...
          q"org.opengrabeso.airframe.surface.LazySurface(classOf[${t}], ${fullTypeNameOf(t)})"
        }
      } else {
        seen += t
        ...
        val surfaceGenerator =
          surfaceFactories andThen { tree =>
            ...
            q"org.opengrabeso.airframe.surface.surfaceCache.getOrElseUpdate(${fullTypeNameOf(t)}, ${tree})"
          }
